...
There is one name string used for all classes and files: CronoACQ
You should replace this by your own name by

$GO4SYS/build/rename.sh "CronoACQ" "myname"
Example:
> $GO4SYS/build/rename.sh "CronoACQ" "Ship"
...
Analysis:  TCronoACQAnalysis

All classes are defined and declared in two files (*.h and *.cxx)
In TCronoACQAnalysis the two steps are created with their factories and input and output
parameters. Here the defaults are set concerning the event IO.
Two parameter objects are created (TCronoACQParameter). They can be used in both steps.

Step one: Unpack

The event filled: TCronoACQUnpackEvent
The processor:    TCronoACQUnpackProc

The factory TCronoACQUnpackFact normally need not to be changed. The TCronoACQUnpackEvent
...
The unpacking code is in the event processor TCronoACQUnpackProc. Members are
histograms, conditions, and parameter pointers used in the event method
CronoACQUnpack. In the constructor of TCronoACQUnpackProc the histograms and
conditions are created, and the pointers to the parameter objects (created in
TCronoACQAnalysis) are set. CronoACQUnpack - called event by event - gets the output
event TCronoACQUnpackEvent as argument (poutevt).
...
The event filled: TCronoACQAnlEvent
The processor:    TCronoACQAnlProc

The step two is build in the same way as step one.
Note that the TCronoACQUnpackEvent is used two times: once as output of step one,
and once as input of step two.
Therefore the Fill method checks if TCronoACQUnpackEvent has to be filled by CronoACQUnpack
...
The user method CronoACQEventAnalysis always gets the pointer to the correct event.
...
from that file.
When TCronoACQAnalysis is created, autosave file is not yet loaded. Therefore are the
objects created here overwritten by the objects from autosave file (if any), except histograms.
The histogram "EventSize" is created in PreLoop of TCronoACQAnalysis, because at this point
...
All arguments, placed after "-args" string will be delivered to TCronoACQAnalysis
...
The main program builds the files names needed and creates the TCronoACQAnalysis.
